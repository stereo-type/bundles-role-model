...
readonly class OperationAddToRole
{
    ...
    public function execute(OperationAddToRoleDTO $dto, bool $flush = true): Role
    {
        $errors = $this->validationService->validateDTO($dto);
        if ($errors->count() > 0) {
            throw BundleException::validationErrors($errors);
        }
        return $this->service->addOperationToRoleId($dto->operationCode, $dto->roleId, $flush);
    }
}
